...
methods
    ...
    function ulg2tt(obj,ulogObj)
        ...
        [obj.AvailableMessages,~,idx] = unique(msg.TopicNames);
        obj.TopicInstances = sum(idx==1:max(idx),1)';

        % Create unique row names
        rowNames = msg.TopicNames;
        for ii = 1:height(msg)
            if msg.InstanceID(ii) > 0
                rowNames(ii) = rowNames(ii)+"_"+num2str(msg.InstanceID(ii));
            end
        end
        msg.Properties.RowNames = rowNames;
        ...
        if strcmp(obj.MessageSet,'Debugging')
            DataLevel = 0;
            DebugLevel = 2;
            Time = seconds(t0_ULOG:dt:tf_ULOG).';
        elseif strcmp(obj.MessageSet,'Basic')
            DataLevel = 0;
            DebugLevel = 0;
            Time = seconds(t0_ULOG:dt:tf_ULOG).';
        elseif strcmp(obj.MessageSet,'Standard')
            DataLevel = 1;
            DebugLevel = 1;
            Time = seconds(t0_ULOG:dt:tf_ULOG).';
        elseif strcmp(obj.MessageSet,'Advanced')
            DataLevel = 2;
            DebugLevel = 2;
            Time = seconds(t0_ULOG:dt:tf_ULOG).';
        elseif strcmp(obj.MessageSet,'Estimation')
            DataLevel = 2;
            DebugLevel = 1;
        ...
        % Data level 0
        obj.GPS(msg);
        obj.VehicleLocalPosition(msg);
        obj.VehicleAttitude(msg);
        obj.BodyVelocity;
        obj.VehicleAngularVelocity(msg);
        obj.VehicleAngularAcceleration(msg);
        obj.AirspeedValidated(msg);
        obj.ActuatorControls(msg);
        obj.ActuatorMotors(msg);
        obj.ActuatorServos(msg);
        %
        % Data Level 1
        if DataLevel > 0
            ...
            obj.InputRC(msg);
            obj.ESCStatus(msg);
            obj.VehicleAirData(msg);
            obj.RPM(msg);
        end
        %
        % Data Level 2
        if DataLevel > 1
            obj.SensorCombined(msg);
            obj.SensorMag(ulog);
            obj.VehicleMagnetometer(msg);
            obj.YawEstimatorStatus(msg);
            obj.EstimatorStates(msg);
            obj.EstimatorStatus(msg);
            obj.Airdata(msg); % Custom
        end
...
methods(Access = private)
    ...
    function GPS(obj,msg)
        ...
        if sensor_gps_avail
            gps_data = msg('sensor_gps',:).TopicMessages{1}(:,{'time_utc_usec',...
                'lat','lon','alt','alt_ellipsoid','s_variance_m_s','eph','epv',...
                'vel_n_m_s','vel_e_m_s','vel_d_m_s'});
            utc_avail = true;
        elseif vehicle_gps_position_avail
            gps_data = msg('vehicle_gps_position',:).TopicMessages{1}(:,{'time_utc_usec',...
                'lat','lon','alt','alt_ellipsoid','s_variance_m_s','eph','epv',...
                'vel_n_m_s','vel_e_m_s','vel_d_m_s'});
            utc_avail = true;
        elseif vehicle_global_position_avail
            gps_data = msg('vehicle_global_position',:).TopicMessages{1}(:,{'lat',...
    ...
    function VehicleLocalPosition(obj,msg)
    ...
    function VehicleAttitude(obj,msg)
        ...
        % Read topic messages
        vehicle_attitude = msg('vehicle_attitude',:).TopicMessages{1};
    ...
    function VehicleAngularVelocity(obj,msg)
    ...
    function VehicleAngularAcceleration(obj,msg)
    ...
    function AirspeedValidated(obj,msg)
    ...
    function ActuatorControls(obj,msg)
        ...
        % Read topic messages
        actuator_controls_0 = msg('actuator_controls_0',:).TopicMessages{1};
    ...
    function ActuatorMotors(obj,msg)
        ...
        % Read topic messages
        actuator_motors = msg('actuator_motors',:).TopicMessages{1};
    ...
    function ActuatorServos(obj,msg)
        ...
        % Read topic messages
        actuator_servos = msg('actuator_servos',:).TopicMessages{1};
    ...
    function VehicleAirData(obj,msg)
    ...
    function SensorCombined(obj,msg)
        ...
        % Read topic messages
        sensor_combined = msg('sensor_combined',:).TopicMessages{1}(:,["gyro_rad","accelerometer_m_s2"]);
    ...
    function VehicleMagnetometer(obj,msg)
        ...
        % Read topic messages
        vehicle_magnetometer = msg('vehicle_magnetometer',:).TopicMessages{1}(:,"magnetometer_ga");
    ...
    function YawEstimatorStatus(obj,msg)
        ...
        % Read topic messages
        yaw_estimator_status = msg('yaw_estimator_status',:).TopicMessages{1};
    ...
    function EstimatorStates(obj,msg)
    ...
    function EstimatorStatus(obj,msg)
