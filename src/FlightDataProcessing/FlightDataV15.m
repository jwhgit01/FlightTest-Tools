...
methods
    ...
    function obj = FlightDataV15(UlogFile)
    ...
    function ulg2tt(obj,ulogObj)
        ...
        % data start and stop times (pad to cut off NaN's)
...
methods(Access = private)
    ...
    function GPS(obj,ulog)
        ...
        if sensor_gps_avail
            gps_data = readTopicMsgs(ulog,"TopicNames","sensor_gps").TopicMessages{1}(:,...
                {'time_utc_usec','latitude_deg','longitude_deg','altitude_msl_m',...
        ...
        if utc_avail
            ...
            obj.Data = addvars(obj.Data,gps_data.zulu_time,gps_data.latitude_deg,...
                gps_data.longitude_deg,gps_data.altitude_msl_m,gps_data.altitude_ellipsoid_m,...
    ...
    function BodyVelocity(obj)
        ...
        for k = 1:N % loop through timetable
            ...
            alpha_derived_rad(k,1) = atan2d(vb_m_s(k,3),vb_m_s(k,1)); % angle of attack (deg)
            beta_derived_rad(k,1) = asind(vb_m_s(k,2)/V_m_s_derived(k,1)); % sideslip (deg)
        end
    ...
    function DifferentialPressure(obj,ulog)

        if ~any(strcmp(obj.AvailableMessages,'differential_pressure'))
            return
        end

        % Loop through all instances of actuator_outputs
        differential_pressure_msgs = readTopicMsgs(ulog,"TopicNames","differential_pressure");
        for ii = 1:height(differential_pressure_msgs)

            % Get topic messages
            instance = differential_pressure_msgs.InstanceID(ii);
            differential_pressure = differential_pressure_msgs.TopicMessages{ii}(:,...
                ["differential_pressure_pa","temperature"]);
    
            % Filter data if applicable
            fs = 1/median(diff(seconds(differential_pressure.timestamp)));
            if ~isempty(obj.CutoffFrequency) && (floor(fs) > 2*obj.CutoffFrequency)
                Wn = 2*obj.CutoffFrequency/fs; % fraction of the Nyquist rate
                [num,den] = butter(5,Wn); % 5th order lowpass filter
                differential_pressure.differential_pressure_pa = filtfilt(num,den,...
                    double(differential_pressure.differential_pressure_pa));
            else
                differential_pressure.differential_pressure_pa = double(differential_pressure.differential_pressure_pa);
            end
    
            % Re-time and add data to timetable
            differential_pressure = retime(differential_pressure,obj.Data.Time,...
                'pchip','EndValues',NaN);
            obj.Data = addvars(obj.Data,differential_pressure.differential_pressure_pa,...
                differential_pressure.temperature,...
                'NewVariableNames',{['differential_pressure_' num2str(instance)],...
                ['temperature_' num2str(instance)]});
    ...
    function SensorFlowAngle(obj,ulog)

        if ~any(strcmp(obj.AvailableMessages,'sensor_flow_angle'))
            return
        end

        % Loop through all instances of actuator_outputs
        sensor_flow_angle_msgs = readTopicMsgs(ulog,"TopicNames","sensor_flow_angle");
        for ii = 1:height(sensor_flow_angle_msgs)

            % Get topic messages
            node_id = sensor_flow_angle_msgs.TopicMessages{ii}.node_id(1);
            sensor_flow_angle = sensor_flow_angle_msgs.TopicMessages{ii}(:,["beta_vane","alpha_vane"]);

            % Remove outliers
            sensor_flow_angle.alpha_vane = filloutliers(sensor_flow_angle.alpha_vane,"pchip","movmedian",5);

            % Uniform sample time before filtering
            dt = median(diff(seconds(sensor_flow_angle.timestamp)));
            sensor_flow_angle = retime(sensor_flow_angle,"regular","linear","TimeStep",seconds(dt));
            ...
            if ~isempty(obj.CutoffFrequency) && (floor(fs) > 2*obj.CutoffFrequency)
                ...
                sensor_flow_angle.alpha_vane = filtfilt(num,den,double(sensor_flow_angle.alpha_vane));
                sensor_flow_angle.beta_vane = filtfilt(num,den,double(sensor_flow_angle.beta_vane));
            end
    
            % Re-time and add data to timetable
            sensor_flow_angle = retime(sensor_flow_angle,obj.Data.Time,...
                'pchip','EndValues',NaN);
            obj.Data = addvars(obj.Data,double(sensor_flow_angle.alpha_vane),double(sensor_flow_angle.beta_vane),...
                'NewVariableNames',{['alpha_vane_' num2str(node_id)],['beta_vane_' num2str(node_id)]});
